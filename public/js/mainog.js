...
function isPaymentPage() {
  return document.getElementById('payment-form') !== null || 
         document.querySelector('.payment-container') !== null; // Added alternative selector
}
...
function saveSearchData() {
  ...
  // Get the actual values from the select elements, not just text content
  const guestsSelect = searchBar.querySelector('select[aria-label="Number of Guests"]');
  const roomsSelect = searchBar.querySelector('select[aria-label="Number of Rooms"]');
  
  // Get numeric values from selects if available
  const guestsValue = guestsSelect ? guestsSelect.value : '2 Guests';
  const roomsValue = roomsSelect ? roomsSelect.value : '1 Room';
  
  // Extract numeric values for use in calculations
  const guestsCount = parseInt(guestsValue) || 2;
  const roomsCount = parseInt(roomsValue) || 1;
  ...
  const searchData = {
    ...
    dates: dates,
    guests: guestsValue,
    guestsCount: guestsCount, // Store the actual number
    rooms: roomsValue,
    roomsCount: roomsCount, // Store the actual number
  ...
  selectButtons.forEach((button, index) => {
    button.addEventListener('click', (event) => {
      ...
      const roomData = {
        ...
        guestsCount: searchData?.guestsCount || 2,
        rooms: searchData?.rooms || '1 Room',
        roomsCount: searchData?.roomsCount || 1
      };
...
function updateYourStayDisplay(searchData) {
  ...
  // Use the stored count values first, then fall back to parsing the text
  const roomsCount = searchData.roomsCount || parseInt(searchData.rooms) || 1;
  const guestsCount = searchData.guestsCount || parseInt(searchData.guests) || 2;
...
function updateRoomSelection(roomData) {
  ...
  // Use the stored count values first, then fall back to parsing the text
  const roomsCount = roomData.roomsCount || parseInt(roomData.rooms) || 1;
  const guestsCount = roomData.guestsCount || parseInt(roomData.guests) || 2;
...
function updateConfirmationDisplay(confirmationData) {
  ...
  // Get the dynamically selected payment method
  const selectedPaymentMethodEl = document.querySelector('.payment-tab.active');
  const dynamicPaymentMethod = selectedPaymentMethodEl 
    ? selectedPaymentMethodEl.textContent.trim() 
    : (confirmationData.paymentMethod || 'Unknown Payment Method');
  ...
  fetch('/api/bookings', {
    ...
    body: JSON.stringify({
      ...
      paymentMethod: dynamicPaymentMethod // Use dynamically selected method
    })
  })
  .then(res => {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Check if we're on a page with the payment form
  if (!paymentForm) {
    // Not on payment page, exit early
    return;
  }

  // Default selected method is the first active tab
  let selectedPaymentMethod = document.querySelector('.payment-tab.active').textContent.trim();
  ...
  if (paymentTabs.length > 0) {
    paymentTabs.forEach(tab => {
      tab.addEventListener('click', function() {
        ...
        selectedPaymentMethod = this.textContent.trim();
      });
    });
  }

  // Payment method display details with masked card/account information
  ...
  paymentForm.addEventListener('submit', function(e) {
    ...
    const displayMethod = paymentDetails[selectedPaymentMethod] || selectedPaymentMethod;
    ...
    // Additional processing for different payment methods
    switch(selectedPaymentMethod) {
      ...
      case 'Net Banking':
        // Potentially add bank selection logic
        break;
      
      case 'UPI':
        // Potentially add UPI ID validation
        break;
    }
  });
});